...
def get_videos(video_all, indices_class, c, n):
    idx_shuffle = np.random.permutation(indices_class[c])[:n]
    return video_all[idx_shuffle]

class NFDataloader:
    def __init__(self, synset, batch_size):
        super().__init__()
    ...
    def __iter__(self):
        size = self.synset.num_classes * self.synset.num_per_class
        num_batch = int(ceil(size / self.batch_size))
        indices = np.arange(size)
        for _ in range(num_batch):
            np.random.shuffle(indices)
            ids = indices[:self.batch_size]
            videos, labels = self.synset.get(indices=ids, need_copy=True)
            yield videos, labels

def evaluate_synset_nf(it_eval, net, synset, testloader, args, mode='hallucinator', return_loss=False, test_freq=None):
    ...
    trainloader = NFDataloader(synset, batch_size=args.batch_train)
        ...
        if (test_freq is None and ep == Epoch) or (test_freq is not None and ep % test_freq == 0 and ep != 0):
            with torch.no_grad():
                ...
                if args.eval_mode != 'top5':
                    save_and_print(args.log_path, '%s Evaluate_%02d: Ep %d time = %ds loss = %.6f train acc = %.2f, test acc = %.2f' % (get_time(), it_eval, ep, int(time.time() - start), loss_train, acc_train*100, acc_test*100))
        if ep in lr_schedule:
            lr *= 0.1
            optimizer = torch.optim.SGD(net.parameters(), lr=lr, momentum=0.9, weight_decay=0.0005)
        if ep % 10 == 0 and args.eval_mode == 'test':
            save_and_print(args.log_path, "Epoch: %d, loss: %.6f, acc_train: %.2f" % (ep, loss_train, acc_train*100))

    return net, acc_train, acc_test

# Below part is same with official code
