...
def get_voxels(voxel_all, indices_class, c, n):
    idx_shuffle = np.random.permutation(indices_class[c])[:n]
    return voxel_all[idx_shuffle]
...
def get_dataset(dataset, data_path, resolution=32, num_workers=0):
    if dataset == 'ModelNet':
        num_classes = 10
        channel = 1
        im_size = (resolution, resolution, resolution)
        class_names = None
        mean = [None]
        std = [None]
        dst_train = ModelNetDataset(data_path,num_classes,split='train',resolution=resolution)
        dst_test = ModelNetDataset(data_path,num_classes,split='test',resolution=resolution)
        ...
        mean = [None]
        std = [None]
        dst_train = ShapeNetDataset(data_path, split='train',resolution=resolution)
        dst_test = ShapeNetDataset(data_path, split='test',resolution=resolution)
...
def get_network(model, channel, num_classes, im_size=(32, 32, 32), dist = True):
    # torch.random.manual_seed(int(time.time() * 1000) % 100000)
    net_width, net_depth, net_act, net_norm, net_pooling = get_default_convnet_setting()

    if model == 'Conv3DNet':
        net = Conv3DNet(channel=channel, num_classes=num_classes, net_width=net_width, net_depth=net_depth, im_size=im_size)
    else:
        net = None
...
def evaluate_synset(it_eval, net, images_train, labels_train, testloader, args, mode='none', return_loss=False, test_freq=None):
...
def rand_cutout(x, param):
    ...
    x = x * mask.unsqueeze(1).unsqueeze(4)
    return x
